"""Fire behaviour calculations for rural areas (grassland with wind reduction factor)
"""
import numpy as np
...
def calc_rate_of_spread(dead_fuel_moisture, wind_speed, curing, wind_reduction_savannah):
    ...
    #Rate of spread (Grazed)
    rate_of_spread = np.empty(np.shape(wind_speed))
    rate_of_spread_1 = (0.054+0.269*wind_speed)*fuel_moisture_factor*curing_factor*M_PER_KM
    rate_of_spread_2 = ((1.1+0.715*np.power(wind_speed-5, 0.844))*
...
def calc_flame_height(rate_of_spread):
    ...
    Matt Plucinski pers. comm. 31/07/2017 (email)- grazed
    """
