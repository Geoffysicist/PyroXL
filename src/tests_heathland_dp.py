from spread_models import heathland
...
num_param_dict = {
    ...
    # 'Curing_SFC': (0,100,25),
    'precipitation': (0,0,1),
    'time_since_rain': (48,48,1),
    'time_since_fire': (25,25,1),
    # 'DF_SFC': (2,10,4),
    # 'SDI_SFC': (0,200,50),
    # 'KBDI_SFC': (0,200,50),
    # 'AWAP_uf': (0, 1, 2.5),
...
output_dict =heathland.calculate(df.to_xarray(),fuel_params_df.iloc[0])
...
print(df.head())
print(df.shape)
df.to_csv('tests/heathland_dp20220929.csv', index=False)
# df.to_pickle('tests/heathland_small.pkl')